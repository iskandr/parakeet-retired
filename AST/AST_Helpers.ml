...
let mk_void_node ?(info=mk_ast_info()) ?(src=SourceInfo.empty) () = 
  mk_node ~info ~src Void 
  
let mk_iftrue_node ?(info=mk_ast_info()) ?(src=SourceInfo.empty) 
   condNode trueNodes =
...
(* Int *) 
let mk_int_node ?(info=mk_ast_info()) ?(src=SourceInfo.empty) x = 
...
let mk_idx_node ?(info=mk_ast_info()) ?(src=SourceInfo.empty) arrNode idx = 
  mk_app_node ~info ~src arrNode [mk_int_node idx]

let mk_eq_node ?(info=mk_ast_info()) ?(src=SourceInfo.empty) lhsNode rhsNode = 
  let eqOp = mk_node $ Prim (Prim.ScalarOp Prim.Eq) in 
  mk_app_node ~info ~src eqOp [lhsNode; rhsNode]

let mk_concat_node ?(info=mk_ast_info()) ?(src=SourceInfo.empty) lhs rhs = 
