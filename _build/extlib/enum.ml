...
let rec map2 f t u =
  if count t <> count u then failwith "Cannot map over different length enums"  
  else {
    count = t.count;
...
let rec filter_map2 f t u =
  let rec next () =
    match f (t.next()) (u.next()) with
    | None -> next ()
    | Some x -> x
    in
  from2 next (fun () -> filter_map2 f (t.clone()) (u.clone()))
  
let keep_mask data mask = 
  filter_map2 (fun x b -> if b then Some x else None) data mask

let remove_mask data mask = 
  filter_map2 (fun x b -> if b then None else Some x) data mask   
