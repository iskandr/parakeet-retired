...
__global__
void reduce2d_kernel(float num_vecs, int vec_len, float *output) {
...
void launch_reduce2d(float *vecs, int num_vecs, int vec_len, float *output) {
  ...
  //                  coalescing. (Actually, since texture fetching is 3x3,
  //                  this push for wider blocks may not be true.)
