...
and eval_exp (expNode : SSA.exp_node) : InterpVal.t list = 
  ...
  | Cast (t, valNode) when Type.is_scalar t -> 
      (match eval_value valNode with 
        | InterpVal.Scalar n -> [InterpVal.Scalar (ParNum.coerce_num n t)]
  ...
  and eval_scalar_op (op : Prim.scalar_op) (args : InterpVal.t list) =
    ...
    *)
    ...
    | Prim.Lt,  [x;y] ->
        InterpVal.of_bool (ParNum.to_float x <= ParNum.to_float y)  
    | Prim.Lte, [x;y] -> 
        InterpVal.of_bool (ParNum.to_float x <= ParNum.to_float y)  
    | Prim.Gt, [x;y] -> 
        InterpVal.of_bool (ParNum.to_float x > ParNum.to_float y)
    | Prim.Gte,  [x;y] -> 
        InterpVal.of_bool (ParNum.to_float x >= ParNum.to_float y)
    | Prim.Div, [x;y] -> 
        InterpVal.of_float (ParNum.to_float x /. ParNum.to_float y)
  ...
  and eval_array_op 
        ...
        (outTypes : Type.t list) : InterpVal.t list =
    (*
    ...
    *) 
    ...
    | Prim.Til, [n] ->
       (* always run on Host *)
       let count =  ParNum.to_int (MemoryState.get_scalar P.memState n) in
...
let eval fundef hostVals =
  ...
  let module E = Mk(struct let memState = memState end) in  
