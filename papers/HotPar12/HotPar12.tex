...
\end{lstlisting}
If the decorator \lstinline{@PAR} were removed, then \lstinline{entropy} will run as ordinary Python code. Since NumPy's operators are statically compiled they can't know if their results will be immediately consumed and thus \lstinline{log2} and element-wise multiplication must allocate intermediate arrays to store their results. By contrast, Parakeet specializes the function\lstinline{entropy} for each distinct input type, optimizes the body into a singled fused reduction and finally executes it as parallel native code.
...
%\item No I/O is allowed.
 \item For a given set of inputs to the function, Parkaeet must be able to infer a unique type for every value in the function's body. For example, expressions such as 
 \lstinline{42 if x else "sausage"} would force Parakeet to abandon type specialization. 
\end{itemize}
