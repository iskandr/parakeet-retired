...
\subsection{Typed Intermediate Language} 
...
It is important to note that specialization is not performed only on types but also also on function tags and the types of their associated closure arguments (since the typed IL cannot express uncertainty about the identity of a function value). One caveat is that non-constant closure values are disallowed. This prevents us from having to implement them as large switch statements on the GPU, which would be very inefficient due to branch divergence. 
