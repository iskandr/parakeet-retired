...
\title{Parakeet: Transparent GPU Acceleration of Dynamic Array Languages}
...
% The idea here is (each paragraph being one of these in turn):
%
% 1. Motivate the reader - what is the issue and why should he care
% 2. State that the issue is important, but that other solutions are lacking in various ways
% 3. State that we solve these problems and a bit about how
% 4. Say how we evaluate ourselves
%
% Thus we have a bam bam bam storyline to why the reader should keep on reading...
%
Contemporary GPUs offer immense performance potential for general purpose computation -- from an average 2.5X speedup over CPUs on 14 standard ``throughput computing'' benchmarks~\cite{Lee10} to 80X on data parallel workloads~\cite{Che09}. However, the commonly used frameworks for general purpose GPU (GPGPU) programming, OpenCL~\cite{Muns10} and NVIDIA's CUDA~\cite{NvidCU}, require highly specialized low level programming and performance tuning knowledge to tap into this potential.

A desire to bring this performance potential to a wider audience has motivated a flurry of recent work on the GPU acceleration of high level languages. 

There has been much recent work on enabling GPU acceleration of higher level languages to ease GPGPU programming and to bring this performance potential to a wider audience.  
% suspicious claim--- array languages are pretty weird themselves 
However, each of these systems either: (1) requires the programmer to learn and use a new domain-specific language~\cite{Chaf11}; 
% better claim but needs to be strongly backed up. Also, knocking on chakravarty in the intro seems like a bad idea since he's heading this conference
(2) is insufficiently expressive for efficiently executing standard nested data parallel programs such as K-Means clustering that are highly amenable to GPU acceleration~\cite{Chak11,Main10}; 
% this can't be our main line of attack against copperhead!
(3) has long compilation times that break the interactivity of the source language~\cite{Cata11}; (
% let's forget jacket 
4) exposes details of the GPU that the programmer must deal with manually~\cite{AcceJa}; or 
% good point but needs to be phrased better
(5) doesn't allow the programmer to use convenient array syntax such as that of NumPy~\cite{AcceJa,Cata11,Chaf11,Chak11,Main10}.

% Ha, no we don't. 
We present Parakeet, the first system that addresses all of the above issues.  

Parakeet is an intelligent runtime library and JIT compiler for array-oriented subsets of high level languages.  Parakeet defines an array-oriented intermediate language and includes a front end for translating programs from source languages to this IL.  The heart of Parakeet is an IL interpreter, which upon reaching an array operator can automatically synthesize and execute a data parallel GPU program to implement that operator.  Parakeet is attached as a library to the source language's interpreter, allowing all of the source language's standard features and tools to be used.
