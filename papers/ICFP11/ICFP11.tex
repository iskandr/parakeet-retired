...
In this section, we discuss the internals of the Parakeet runtime in more detail. We implemented the Parakeet runtime and intepreter in OCaml due to its amenability to compiler programming patterns.  We spent considerable effort on tuning the OCaml code to be efficient and to prevent OCaml's garbage collector from becoming a source of large overheads. We extend the intepreter via an interface written in C to allow it to interact with CUDA and with NVIDIA's GPU JIT compiler.  Parakeet also exposes a front end interface written in C that the source language uses to translate functions into Parakeet ASTs and to pass data to and from the Parakeet runtime.
...
Should talk about the recent Stanford DSML PPoPP paper~\cite{Chaf11}.
