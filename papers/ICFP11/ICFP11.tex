...
%
Contemporary GPUs offer immense performance potential for general purpose computation -- from an average 2.5X speedup over CPUs on 14 standard ``throughput computing'' benchmarks~\cite{Lee10} to 80X on data parallel workloads~\cite{Che09}.  Further, most desktop machines today include underutilized graphics accelerators.  However, the widespread frameworks for general purpose GPU (GPGPU) programming, OpenCL~\cite{Muns10} and NVIDIA's CUDA~\cite{NvidCU}, require highly specialized low level programming and performance tuning knowledge to tap into this potential.

There has been much recent work on enabling GPU acceleration of higher level languages to ease GPGPU programming and to bring this performance potential to a wider audience.  However, each of these systems either: (1) requires the programmer to learn and use a new domain-specific language~\cite{Chaf11}; (2) is insufficiently expressive for efficiently executing standard nested data parallel programs such as K-Means clustering that are highly amenable to GPU acceleration~\cite{Chak11,Main10}; (3) has long compilation times that break the interactivity of the source language~\cite{Cata11}; (4) exposes details of the GPU that the programmer must deal with manually~\cite{AcceJa}; or (5) doesn't allow the programmer to use convenient array syntax such as that of NumPy~\cite{AcceJa,Cata11,Chaf11,Chak11,Main10}.

We present Parakeet, the first system that addresses all of the above issues.  Parakeet is an intelligent runtime library and JIT compiler for array-oriented subsets of high level languages.  Parakeet defines an array-oriented intermediate language and includes a front end for translating programs from source languages to this IL.  The heart of Parakeet is an IL interpreter, which upon reaching an array operator can automatically synthesize and execute a data parallel GPU program to implement that operator.  Parakeet is attached as a library to the source language's interpreter, allowing all of the source language's standard features and tools to be used.

We evaluate Parakeet on two standard benchmarks: Black-Scholes option pricing, and K-Means clustering.  We compare high level array-oriented implementations to hand-written, tuned GPU versions from the CUDA SDK~\cite{NvidSD} and the Rodinia GPU benchmark suite~\cite{Che09}. Despite having orders of magnitude less code, the high level versions perform competitively when executed by Parakeet.
\end{abstract}
