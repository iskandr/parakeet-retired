...
(* Use this function for verifying the heap's integrity *)
let checkpoint p =
  Gc.compact ();
  prerr_endline ("Heap verified at position " ^ p)
...
let debug_gen_module_template entries =
  ...
  | [] ->
    checkpoint "gen_module_template";
    Int64.one
...
let aux_ktypenum_to_ocaml_type ktypenum =
  ...
  | Float32T -> printf "Got Float32T for %s\n%!" (string_of_int ktypenum)
  | Float64T -> printf "Got Float64T for %s\n%!" (string_of_int ktypenum)
